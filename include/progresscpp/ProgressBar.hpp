...
class ProgressBar {
 ...
 public:
  ProgressBar(std::uint64_t total,
              std::uint64_t width,
              char complete,
              char incomplete)
      : total_ticks{total},
  ...
  ProgressBar(std::uint64_t total, std::uint64_t width)
  ...
  void display() const {
    float progress =
        static_cast<float>(ticks) / static_cast<float>(total_ticks);
    std::uint64_t pos =
        static_cast<std::uint64_t>(static_cast<float>(bar_width) * progress);
